 ...
 * Implement
 */
print_r("<pre>");var_dump(index_data());
/**
...
function create_tdm_product_index_table($do_truncate = true){
    ...
    if($do_truncate !== false){
        $truncate = "TRUNCATE TABLE `tdm_product_index`; TRUNCATE TABLE `tdm_product_match`";
        $con->query($truncate);
    }
    $con->close();
    return true;
...
function get_ssa_price_and_currency_by_model_and_condition($model,$condition){
    ...
    $query = "SELECT `recycler_product`.`currency` AS `currency`, `recycler_product`.`price` AS `price` FROM `recycler_product` WHERE `condition_id` = '$condition' AND `model` = '$model' AND `recycler_id` = '1'";
    $result = $con->query($query);
...
function get_current_exchange_rate($currency,$time = null){
    global $host,$user,$password,$dbname;
    $con = new mysqli($host,$user ,$password ,$dbname);
    if (mysqli_connect_errno($con)){
        echo "Failed to connect to MySQL: " . mysqli_connect_error();
    ...
    $query = "SELECT `m`.`exchange_rate` AS `exchange_rate` FROM `exchange` AS `m` WHERE m.currency = '$currency' AND m.time <= $time ORDER BY `m`.`time` DESC LIMIT 1";
}
function index_data(){
    global $host,$user,$password,$dbname;
    $con = new mysqli($host,$user ,$password ,$dbname);
